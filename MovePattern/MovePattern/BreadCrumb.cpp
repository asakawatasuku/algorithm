...
void BreadCrumb::Initialize(Base* object, Base* target)
{
	...
	m_object->SetSpeed(Vector3::Forward * 0.01f);

	m_bread_array = new Vector3();

	m_pos = m_object->GetPos();
	direction = m_object->GetRot();
}
...
void BreadCrumb::Update()
{
	...
	{
		if (LMath::IsCollisionCircle(m_target_bread, m_pos, BREAD_COLLISION_RANGE))
		{
			m_target_bread.x = -1.0f;
			m_target_bread.z = -1.0f;
			//m_object->SetSpeed(Vector3::Forward * 0.0f);
		}
	}
	...
	if (!(m_target_bread.x >= 0.0f && m_target_bread.z >= 0.0f))
	{
		for (int i = 0; i < m_bread_num; i++)
		{
			...
			if (!LMath::IsCollisionCircle(bread_pos, m_pos, r)) 
			{
				continue;
			}
	...
	if (m_target_bread.x >= 0.0f && m_target_bread.z >= 0.0f)
	{
		Vector3 dir_vec = LMath::Normalize(m_pos, m_target_bread);

		direction.y = ADJUST_RAD(atan2f(-dir_vec.y, dir_vec.x));
	}
	// 目標のパンくずがないならランダムで移動
	else
	{
	...
	m_pos.x += Vector3::Distance(Vector3::Zero, m_object->GetSpeed()) * cosf(direction.y);
	m_pos.y += Vector3::Distance(Vector3::Zero, m_object->GetSpeed()) * -sinf(direction.y);
...
void BreadCrumb::DropBreadCrumb()
{
	...
	if (LMath::IsCollisionCircle(m_bread_array[0], m_pos, BREAD_CRUMB_MAX_RANGE))
	{
		return;
	}
	// パンくずの鮮度更新
